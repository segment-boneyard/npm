...
cache.add = function (pkg, ver, scrub, cb) {
  ...
  return _add([pkg, ver], cb)
}

function _add(args, cb) {
  var name, spec;
  ...
  if (0 == spec.indexOf('s-')) {
    var version = spec.split('@')[1] || '*';
    var name = spec.split('@')[0];
    var repo = 'segmentio/' + name.slice(2);

    var user = npm.config.get('github_user');
    var pass = npm.config.get('github_pass');
    ...
    lookup({
      ...
      if (!release) return cb(new Error('failed to find release satisfying ' + spec));
      var url = release.tarball_url;
